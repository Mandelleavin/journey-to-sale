...
import { Rocket, Save, FileText, GraduationCap } from "lucide-react";
...
type ProductKey = "ebook" | "kurs";
const PRODUCTS: Record<ProductKey, { label: string; price: number; icon: typeof FileText }> = {
  ebook: { label: "Ebook", price: 97, icon: FileText },
  kurs: { label: "Kurs", price: 1490, icon: GraduationCap },
};
                  ...
                  >
                    <div
                      className={`w-10 h-10 rounded-xl grid place-items-center ${
                        active ? "bg-gradient-violet text-primary-foreground" : "bg-muted text-foreground"
                      }`}
                    >
                      <Icon className="w-5 h-5" />
                    </div>
                    <div className="font-display font-extrabold mt-2">{p.label}</div>
                    <div className="text-xs text-muted-foreground">{p.price} zł</div>
                  </button>
                );
              })}
            </div>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl bg-gradient-to-br from-violet-soft to-blue-soft border border-border p-5">
            ...
            <div className="text-xs text-muted-foreground mt-2">
              {qty} × {PRODUCTS[product].label} ({price} zł)
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3 text-sm">
            <Mini label="Cena jednostkowa" value={`${price} zł`} />
            <Mini label="Sprzedanych sztuk" value={String(qty)} />
          </div>
          <p className="text-xs text-muted-foreground">
            Wskazówka: nawet kilkanaście sprzedaży kursu = poważny zastrzyk gotówki na start
            kolejnego produktu.
          </p>
        </div>
      }
      history={
        <ul className="text-sm divide-y divide-border">
          {(hist?.rows ?? []).map((r) => {
            ...
            const i = r.inputs as { product?: string; qty?: number };
            return (
