...
export function MentorTasksSection() {
  ...
  const submit = async () => {
    ...
    const { error } = await supabase
      .from("mentor_assigned_tasks")
      .update({ submission_content: content, status: "submitted", submitted_at: new Date().toISOString() })
      .eq("id", active.id);
    setSubmitting(false);
    if (error) toast.error(error.message);
    else { toast.success("Wysłano do mentora"); setActive(null); load(); }
  };
    ...
    <section className="bg-card rounded-3xl border border-border shadow-card p-5 lg:p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Spersonalizowane zadania przypisane bezpośrednio dla Ciebie</p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {tasks.map((t) => {
          ...
          return (
            <div key={t.id} className="rounded-2xl border border-border p-4 hover:border-violet/40 transition-colors">
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-sm">{t.title}</div>
                  {t.instructions && <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{t.instructions}</p>}
                  <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                    <span className="inline-flex items-center gap-1 font-bold text-violet"><Zap className="w-3 h-3" />+{t.xp_reward} XP</span>
                    {t.due_date && <span className="inline-flex items-center gap-1"><Calendar className="w-3 h-3" />{new Date(t.due_date).toLocaleDateString("pl-PL")}</span>}
                  </div>
                </div>
                <div className="flex flex-col items-end gap-2">
                  <Badge variant="outline" className={cn("text-[10px]", tag.tone)}>{tag.label}</Badge>
                  {interactive && (
                    <Button size="sm" onClick={() => open(t)} className="bg-gradient-violet text-primary-foreground h-7 text-xs">
      ...
      <Dialog open={!!active} onOpenChange={(v) => !v && setActive(null)}>
        <DialogContent>
          ...
          {active?.instructions && <p className="text-sm text-muted-foreground">{active.instructions}</p>}
          <Textarea
          ...
          />
          <DialogFooter>
            <Button variant="outline" onClick={() => setActive(null)}>Anuluj</Button>
            <Button onClick={submit} disabled={submitting} className="bg-gradient-violet text-primary-foreground">
