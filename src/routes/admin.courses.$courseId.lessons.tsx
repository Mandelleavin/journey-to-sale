...
type Lesson = { id: string; title: string; module_id: string | null; position: number; xp_reward: number; is_published: boolean };
...
function AllLessonsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ data: c }, { data: m }, { data: l }] = await Promise.all([
        ...
        supabase.from("lessons").select("id, title, module_id, position, xp_reward, is_published").eq("course_id", courseId).order("position"),
      ]);
  ...
  if (loading || !isAdmin) return <div className="grid min-h-screen place-items-center bg-app">Ładowanie...</div>;
  ...
  return (
    <div className="min-h-screen bg-app">
      <div className="mx-auto max-w-[1100px] p-4 md:p-6">
        ...
        <h1 className="font-display text-3xl font-extrabold mt-2 mb-6">Wszystkie lekcje: {course?.title}</h1>

        <div className="space-y-4">
          {modules.map((m) => {
            const inMod = lessons.filter((l) => l.module_id === m.id);
            return (
              <Link key={m.id} to="/admin/modules/$moduleId" params={{ moduleId: m.id }} className="block rounded-2xl border border-border bg-card p-4 hover:border-violet/40">
          ...
          {orphans.length > 0 && (
            <div className="rounded-2xl border border-orange/40 bg-orange-soft/30 p-4">
              <div className="font-bold flex items-center gap-2 mb-2"><BookOpen className="w-4 h-4" /> Lekcje bez modułu ({orphans.length})</div>
              <p className="text-xs text-muted-foreground mb-3">Te lekcje nie są przypisane do żadnego modułu. Edytuj je i przypisz moduł.</p>
              <ul className="text-sm space-y-1">
                {orphans.map((l) => <li key={l.id}>• {l.title}</li>)}
              </ul>
            </div>
          )}
          {modules.length === 0 && orphans.length === 0 && (
            <div className="text-sm text-muted-foreground p-6 text-center">Brak lekcji w tym kursie.</div>
          )}
        </div>
      </div>
    </div>
