...
import { askCoach } from "@/lib/gamification.functions";
...
function CoachPage() {
  ...
  const [history, setHistory] = useState<Msg[]>([
    {
      role: "assistant",
      content:
        "CzeĹ›Ä‡! Jestem Twoim AI Coachem. Powiedz, w czym mogÄ™ pomĂłc â€” strategia, oferta, marketing, kolejny krok?",
    },
  ]);
  ...
  return (
    <PageShell
      title="AI Coach"
      subtitle="Spersonalizowane wskazĂłwki na bazie Twojego profilu i postÄ™pĂłw"
    >
      <div className="rounded-3xl border border-border bg-card shadow-soft flex flex-col h-[600px]">
        <div className="flex items-center gap-2 p-4 border-b border-border">
          ...
          <div className="flex-1">
            <div className="font-display font-bold">TwĂłj Coach</div>
            <div className="text-xs text-muted-foreground">
              {remaining !== null
                ? `${remaining} wiadomoĹ›ci pozostaĹ‚o dziĹ›`
                : "Limit 20 wiadomoĹ›ci / dzieĹ„"}
            </div>
          </div>
        </div>
