    ...
    <div className="min-h-screen bg-app">
      ...
      <Dialog open={!!editingCourse} onOpenChange={(v) => !v && setEditingCourse(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingCourse?.id ? "Edytuj kurs" : "Nowy kurs"}</DialogTitle>
          </DialogHeader>
          {editingCourse && (
            <div className="space-y-3">
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    type="number"
                    value={editingCourse.required_xp ?? 0}
                    onChange={(e) =>
                      setEditingCourse({
                        ...editingCourse,
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEditingCourse({ ...editingCourse, cover_url: e.target.value })
                    }
                  />
                </div>
              </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={saveCourse} className="bg-gradient-violet text-primary-foreground">
              Zapisz
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={!!editingModule} onOpenChange={(v) => !v && setEditingModule(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingModule?.module.id ? "Edytuj moduł" : "Nowy moduł"}</DialogTitle>
          </DialogHeader>
          {editingModule && (
            <div className="space-y-3">
              <div>
              ...
              <div>
                ...
                <Input
                  type="number"
                  min={0}
                  value={editingModule.module.unlock_after_hours ?? 0}
                  onChange={(e) =>
                    setEditingModule({
                      ...editingModule,
                      module: {
                        ...editingModule.module,
                        unlock_after_hours: parseInt(e.target.value) || 0,
                      },
                    })
                  }
                />
                <p className="text-xs text-muted-foreground mt-1">
                  0 = od razu. Np. 48 = po 2 dniach.
                </p>
              </div>
              <div className="flex items-center justify-between rounded-xl bg-muted/30 px-3 py-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Lekcje będą zablokowane, dopóki użytkownik nie zaliczy wszystkich lekcji
                    poprzedniego modułu
                  </p>
                </div>
                <Switch
                  checked={editingModule.module.requires_previous_module ?? false}
                  onCheckedChange={(v) =>
                    setEditingModule({
                      ...editingModule,
                      module: { ...editingModule.module, requires_previous_module: v },
                    })
                  }
                />
              </div>
              <div className="flex items-center justify-between rounded-xl bg-muted/30 px-3 py-2">
                <Label>Opublikowany</Label>
                <Switch
                  checked={editingModule.module.is_published ?? true}
                  onCheckedChange={(v) =>
                    setEditingModule({
                      ...editingModule,
                      module: { ...editingModule.module, is_published: v },
                    })
                  }
                />
              </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={saveModule} className="bg-gradient-violet text-primary-foreground">
              Zapisz
            </Button>
          </DialogFooter>
        </DialogContent>
