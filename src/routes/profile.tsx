  ...
  return (
    <div className="min-h-screen bg-app">
      <div className="mx-auto max-w-3xl p-4 md:p-6">
        ...
        <p className="text-sm text-muted-foreground mb-6">Zarządzaj danymi, hasłem i przeglądaj postępy</p>

        <Tabs defaultValue="info">
          <TabsList className="grid grid-cols-4 max-w-lg">
            <TabsTrigger value="info"><User className="w-4 h-4 mr-1" />Dane</TabsTrigger>
            <TabsTrigger value="password"><KeyRound className="w-4 h-4 mr-1" />Hasło</TabsTrigger>
            <TabsTrigger value="xp"><Zap className="w-4 h-4 mr-1" />XP</TabsTrigger>
            <TabsTrigger value="survey"><ClipboardList className="w-4 h-4 mr-1" />Ankieta</TabsTrigger>
          </TabsList>

          <TabsContent value="info" className="mt-6"><InfoTab /></TabsContent>
          <TabsContent value="password" className="mt-6"><PasswordTab /></TabsContent>
          <TabsContent value="xp" className="mt-6"><XpTab /></TabsContent>
...
function InfoTab() {
  ...
  return (
    <div className="rounded-3xl border border-border bg-card p-6 space-y-4">
      ...
      <Button onClick={save} disabled={saving} className="bg-gradient-violet text-primary-foreground">
...
function PasswordTab() {
  ...
  const save = async () => {
    ...
    setSaving(false);
    if (error) toast.error(error.message);
    else { toast.success("Hasło zmienione"); setPw(""); setPw2(""); }
  };
...
function XpTab() {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("user_xp_log").select("id, amount, reason, created_at")
        ...
        {rows.map((r) => (
          <li key={r.id} className="py-2.5 flex items-center justify-between text-sm">
            <div>
              ...
              <div className="text-xs text-muted-foreground">{new Date(r.created_at).toLocaleString("pl-PL")}</div>
            </div>
            <span className="font-bold text-green">+{r.amount}</span>
          </li>
        ))}
        {rows.length === 0 && <li className="py-6 text-center text-sm text-muted-foreground">Brak wpisów XP</li>}
      </ul>
    </div>
  );
}
...
function SurveyTab() {
  return (
    <div className="rounded-3xl border border-border bg-card p-6 text-center">
      <p className="text-sm text-muted-foreground mb-4">Możesz ponownie wypełnić ankietę profilującą — wynik nadpisze poprzedni.</p>
      <Link to="/onboarding">
        <Button className="bg-gradient-violet text-primary-foreground">Wypełnij ankietę ponownie</Button>
      </Link>
    </div>
  );
}
