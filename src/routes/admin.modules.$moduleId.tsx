...
function AdminModuleLessonsPage() {
  ...
  const load = async () => {
    ...
    if (lessonIds.length > 0) {
      ...
      (t ?? []).forEach((task) => {
        (byL[(task as LessonTask).lesson_id] ||= []).push(task as LessonTask);
      });
    ...
    <div className="min-h-screen bg-app">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 p-4">
                    ...
                    <button
                      onClick={() => setActiveLessonId(isActive ? null : l.id)}
                    ...
                    >
                      ...
                      <div className="text-xs text-muted-foreground mt-0.5">
                        +{l.xp_reward} XP · {tasks.length} zadań · {attachments.length} załączników
                        · {l.content_blocks.length} bloków
                      </div>
                    </button>
                  ...
                  {isActive && (
                    <div className="border-t border-border bg-muted/20 p-4 space-y-4">
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="text-sm font-bold flex items-center gap-1.5">
                            <ListChecks className="w-4 h-4" /> Zadania po lekcji
                          </h4>
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() =>
                              setEditingTask({
                                lessonId: l.id,
                                task: { title: "", xp_reward: 50, is_required: false },
                              })
                            }
                          >
                          ...
                          <div className="space-y-1.5">
                            {tasks.map((t) => (
                              ...
                              >
                                ...
                                <div className="text-xs font-bold text-violet shrink-0">
                                  +{t.xp_reward} XP
                                </div>
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => setEditingTask({ lessonId: l.id, task: t })}
                                >
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="text-sm font-bold flex items-center gap-1.5">
                            <Paperclip className="w-4 h-4" /> Materiały (PDF, pliki)
                          </h4>
                          <label className="cursor-pointer">
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                if (f) uploadAttachment(l.id, f);
                                e.target.value = "";
                              }}
                            />
                            <span className="inline-flex items-center text-xs font-bold border border-input rounded-md px-3 py-1.5 hover:bg-muted">
                              <Plus className="w-3.5 h-3.5 mr-1" /> Dodaj plik
                            </span>
                          </label>
                        </div>
                            ...
                            {attachments.map((a) => (
                              ...
                              >
                                ...
                                <a
                                  ...
                                  className="text-sm font-bold text-violet hover:underline flex-1 truncate"
                                >
                                  {a.title}
                                </a>
                                <Button
      ...
      <Dialog open={!!editing} onOpenChange={(v) => !v && setEditing(null)}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing?.id ? "Edytuj lekcję" : "Nowa lekcja"}</DialogTitle>
          </DialogHeader>
          {editing && (
            <div className="space-y-3">
              <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <Label>Drip (godz.)</Label>
                  <Input
                    type="number"
                    min={0}
                    value={editing.unlock_after_hours ?? 0}
                    onChange={(e) =>
                      setEditing({ ...editing, unlock_after_hours: parseInt(e.target.value) || 0 })
                    }
                  />
                </div>
                <div className="flex items-end">
                  <div
                    className={cn(
                      "flex items-center justify-between rounded-xl bg-muted/30 px-3 py-2 w-full",
                    )}
                  >
                    <Label className="text-xs">Opublikowana</Label>
                    <Switch
              ...
              <div className="flex items-center justify-between rounded-xl bg-muted/30 px-3 py-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Następna lekcja będzie zamknięta, dopóki użytkownik nie zaliczy zadania z tej
                  </p>
                </div>
                <Switch
                  checked={editing.requires_task_completion ?? false}
                  onCheckedChange={(v) => setEditing({ ...editing, requires_task_completion: v })}
                />
              </div>
              <div>
                ...
                <div className="mt-2">
                  <ContentBlocksEditor
                    value={editing.content_blocks ?? []}
                    onChange={(blocks) => setEditing({ ...editing, content_blocks: blocks })}
                  />
                </div>
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={saveLesson} className="bg-gradient-violet text-primary-foreground">
              Zapisz
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={!!editingTask} onOpenChange={(v) => !v && setEditingTask(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingTask?.task.id ? "Edytuj zadanie" : "Nowe zadanie"}</DialogTitle>
          </DialogHeader>
          {editingTask && (
            <div className="space-y-3">
              <div>
              ...
              <div>
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setEditingTask({
                      ...editingTask,
                      task: { ...editingTask.task, instructions: e.target.value },
                    })
                  }
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  value={editingTask.task.xp_reward ?? 50}
                  onChange={(e) =>
                    setEditingTask({
                      ...editingTask,
                      task: { ...editingTask.task, xp_reward: parseInt(e.target.value) || 0 },
                    })
                  }
                />
              </div>
              <div className="flex items-center justify-between rounded-xl bg-muted/30 px-3 py-2">
                ...
                <Switch
                  checked={editingTask.task.is_required ?? false}
                  onCheckedChange={(v) =>
                    setEditingTask({
                      ...editingTask,
                      task: { ...editingTask.task, is_required: v },
                    })
                  }
                />
              </div>
